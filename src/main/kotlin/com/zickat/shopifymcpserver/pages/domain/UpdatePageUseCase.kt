...
class UpdatePageUseCase(
    ...
    fun execute(storeId: String, pageId: String, title: String?, body: String?, handle: String?): Either<UseCaseError, UpdatePageResult> =
        if (title == null && body == null && handle == null) {
            UpdatePageResult.NoOp.right()
        } else {
            either {
                if (pageRepository.get(storeId, pageId).bind() == null) {
                    UpdatePageResult.notFound(pageId)
                } else {
