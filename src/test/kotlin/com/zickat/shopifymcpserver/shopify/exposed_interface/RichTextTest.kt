...
class RichTextTest {
    ...
    @Test
    fun `toPlainText should return empty for an empty string`() {
        RichText.toPlainText("") shouldBe ""
    }
    ...
    @Test
    fun `toPlainText should return empty for a technically non-null JSON that carries no readable text`() {
        val emptyDocument = """{"type":"root","children":[]}"""

        RichText.toPlainText(emptyDocument) shouldBe ""
    }

    @Test
    fun `toPlainText should return empty when the only paragraph carries an empty text value`() {
        val blankParagraph = """{"type":"root","children":[{"type":"paragraph","children":[{"type":"text","value":""}]}]}"""

        RichText.toPlainText(blankParagraph) shouldBe ""
    }
    ...
    @Test
    fun `parseStringArray should return an empty list when the JSON is not an array`() {
        RichText.parseStringArray("""{"not":"an array"}""") shouldBe emptyList()
    }
