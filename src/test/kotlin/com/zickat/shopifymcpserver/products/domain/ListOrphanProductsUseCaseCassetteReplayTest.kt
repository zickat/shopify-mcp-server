...
/**
 * Exercises `list_orphan_products` against a real `ListOrphanProductsUseCase`, replaying the
 * `LOT5-01` cassettes — each carrying the tool's two-scan sequence (collections, then products).
 */
class ListOrphanProductsUseCaseCassetteReplayTest {
    ...
    private fun useCaseReplaying(cassette: Cassette, storeId: String, shopDomain: String): Pair<ListOrphanProductsUseCase, MockWebServer> {
        ...
        return ListOrphanProductsUseCase(ProductShopifyRepository(gateway)) to mockServer
    }
    ...
    private fun replayAndAssert(cassetteResource: String, storeId: String, shopDomain: String, storeSlug: String) {
        ...
        // when
        val result = useCase.execute(storeId).shouldBeRight()
        ...
        val rendered = ProductsToolResults.listOrphanProductsResult(storeSlug, result)
        rendered.content().map { (it as TextContent).text() } shouldBe expectedTexts(cassette)
    }

    @Test
    fun `replaying the velotrip empty cassette across its collections- and products-scan calls reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
            "cassettes/list-orphan-products-velotrip-empty.json",
            "store-velotrip",
    ...
    @Test
    fun `replaying the lurelab normal cassette across its collections- and products-scan calls reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
            "cassettes/list-orphan-products-lurelab-normal.json",
            "store-lurelab",
