...
/**
 * Exercises the whole `LOT4-01` cassette-recorded behaviour of `list_metaobjects` (both the
 * without-type and the with-type form) against a real `ListMetaobjectsUseCase` — every expected
 * response is read straight from the cassette's own `toolOutput`, never restated by hand, so a
 * corrupted cassette fails this test rather than passing silently — see the tautology check
 * recorded in progress.md for how that guarantee was verified.
 */
class ListMetaobjectsUseCaseCassetteReplayTest {
    ...
    private fun useCaseReplaying(cassette: Cassette, storeId: String, shopDomain: String): Pair<ListMetaobjectsUseCase, MockWebServer> {
        ...
        return ListMetaobjectsUseCase(metaobjectsRepository) to mockServer
    }
    ...
    private fun replayAndAssert(cassetteResource: String, storeId: String, shopDomain: String, storeSlug: String) {
        ...
        val type = cassette.toolInput.jsonObject["type"]?.jsonPrimitive?.content

        // when
        val result = useCase.execute(storeId, type).shouldBeRight()
        ...
        val rendered = MetaobjectToolResults.listMetaobjectsResult(storeSlug, result)
        rendered.content().map { (it as TextContent).text() } shouldBe expectedTexts(cassette)
    }

    @Test
    fun `replaying the velotrip no-type cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
    ...
    @Test
    fun `replaying the lurelab guide_theme typed cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
