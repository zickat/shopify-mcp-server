...
/**
 * Exercises the whole `LOT4-01` cassette-recorded behaviour of `get_metaobject` (referenced,
 * orphan and not-found) against a real `GetMetaobjectUseCase` — every expected response is read
 * straight from the cassette's own `toolOutput`, never restated by hand, so a corrupted cassette
 * fails this test rather than passing silently — see the tautology check recorded in progress.md
 * for how that guarantee was verified.
 */
class GetMetaobjectUseCaseCassetteReplayTest {
    ...
    private fun useCaseReplaying(cassette: Cassette, storeId: String, shopDomain: String): Pair<GetMetaobjectUseCase, MockWebServer> {
    ...
    private fun replayAndAssert(cassetteResource: String, storeId: String, shopDomain: String, storeSlug: String) {
        ...
        val metaobjectId = cassette.toolInput.jsonObject.getValue("metaobject_id").jsonPrimitive.content

        // when
        val result = useCase.execute(storeId, metaobjectId).shouldBeRight()
    ...
    @Test
    fun `replaying the lurelab referenced cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
    ...
    @Test
    fun `replaying the lurelab orphan cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
