...
/**
 * Exercises the whole `LOT4-01` cassette-recorded behaviour of `list_pages` (without query and with
 * a query filtering to a subset) against a real `ListPagesUseCase` — every expected response is read
 * straight from the cassette's own `toolOutput`, never restated by hand, so a corrupted cassette
 * fails this test rather than passing silently — see the tautology check recorded in progress.md for
 * how that guarantee was verified.
 */
class ListPagesUseCaseCassetteReplayTest {
    ...
    private fun useCaseReplaying(cassette: Cassette, storeId: String, shopDomain: String): Pair<ListPagesUseCase, MockWebServer> {
        ...
        return ListPagesUseCase(pageRepository) to mockServer
    }
    ...
    private fun replayAndAssert(cassetteResource: String, storeId: String, shopDomain: String, storeSlug: String) {
        ...
        val query = cassette.toolInput.jsonObject["query"]?.jsonPrimitive?.content

        // when
        val result = useCase.execute(storeId, query).shouldBeRight()
        ...
        val rendered = PageToolResults.listPagesResult(storeSlug, result)
        rendered.content().map { (it as TextContent).text() } shouldBe expectedTexts(cassette)
    }

    @Test
    fun `replaying the velotrip no-query cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
    ...
    @Test
    fun `replaying the velotrip bikepacking-query cassette reproduces the recorded toolOutput bit for bit`() {
        replayAndAssert(
